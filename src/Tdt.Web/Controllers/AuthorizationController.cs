...
//            ticket.SetResources(request.GetResources());
            ...
            foreach (var claim in ticket.Principal.Claims)
            {
                ...
                if ((claim.Type == OpenIdConnectConstants.Claims.Subject && ticket.HasScope(OpenIdConnectConstants.Scopes.OpenId)) ||
                    ...
                    (claim.Type == "permission" && ticket.HasScope(OpenIddictConstants.Claims.Roles)))
                {
            ...
            if (ticket.HasScope(OpenIdConnectConstants.Scopes.Profile))
            {
                if (!string.IsNullOrWhiteSpace(user.AdSoyad))
                    identity.AddClaim("fullName", user.AdSoyad, OpenIdConnectConstants.Destinations.IdentityToken);
